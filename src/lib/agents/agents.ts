...
import {getGeminiProModel, getGeminiFlashModel} from "./models-pool";

// --- Diagram generation system prompt, used as LLM system prompt
...
// Diagram generation agent (Pro model + diagram system prompt).
export async function getDiagramModel(sessionId?: string) {
    return createAgent({
        model: getGeminiProModel(sessionId),
        tools: [],
        systemPrompt: DIAGRAM_MODEL_SYSTEM_PROMPT,
    });
}

// Code generation agent (Flash model + code system prompt + optional session context).
export async function getCodeModel(
    ...
    return createAgent({
        model: getGeminiFlashModel(sessionId),
        tools: [],
...
// Helper function. It converts stored conversation history to LangChain message objects.
